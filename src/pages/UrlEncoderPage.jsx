...
export default function UrlEncoderPage() {
  const tool = allTools.find(t => t.id === 'url-encoder')
  if (!tool) return null
  ...
  const schemaData = {
    ...
    "url": "https://instant-dev-tool-converter.vercel.app/url-encoder"
  }

  return (
    <div className="app" style={{ overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
      <Helmet>
        <title>Free URL Encoder & Decoder Online – InstantConverters</title>
        <meta name="description" content="Encode or decode URL strings safely. Essential web tool for parsing queries, escaping special characters, and testing HTTP requests." />
        <meta name="keywords" content="URL encoder" />
      ...
      <div style={{ flexShrink: 0, maxWidth: 1200, width: '100%', margin: '0 auto', padding: '24px 24px 0' }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--t3)', marginBottom: 0, lineHeight: 1.5 }}>
          Encode or decode URL strings safely. Essential web tool for parsing queries, escaping special characters, and testing HTTP requests.
        </p>
      </div>
      ...
      <section style={{ flexShrink: 0, maxWidth: 1200, width: '100%', margin: '24px auto 0', padding: '0 24px' }} className="space-y-4 seo-content-section">
        <h2 style={{ fontSize: 16, fontWeight: 600, color: 'var(--t1)', marginBottom: 8 }}>What is a URL Encoder & Decoder?</h2>
        <p style={{ fontSize: 14, color: 'var(--t3)', marginBottom: 24, lineHeight: 1.6 }}>
          {"URL encoding (percent-encoding) converts special characters into a format that can be safely transmitted over the Internet. It prevents special characters from breaking HTTP requests."}
        </p>

        <h2 style={{ fontSize: 16, fontWeight: 600, color: 'var(--t1)', marginBottom: 8 }}>How to use this tool</h2>
        <p style={{ fontSize: 14, color: 'var(--t3)', marginBottom: 24, lineHeight: 1.6 }}>
          {"Paste a raw URL or query parameter. Click Encode to make it web-safe, or Decode to convert a percent-encoded string back to normal text."}
        </p>
        ...
        <pre style={{ fontSize: 14, color: 'var(--t3)', marginBottom: 24, lineHeight: 1.6, fontFamily: 'var(--mono)', background: 'var(--inp)', padding: '12px', borderRadius: '6px', border: '1px solid var(--bdr)', whiteSpace: 'pre-wrap' }}>
          {"Input: /path?name=john doe → Output: %2Fpath%3Fname%3Djohn%20doe"}
        </pre>
      </section>
